...
export default function Navbar() {
  ...
  if (pathname === '/verificar-edad') return null

  return (
    <nav className="sticky top-0 z-50 w-full border-b border-transparent bg-[#030712]/80 backdrop-blur-md shadow-sm [border-image:linear-gradient(to_right,#3b82f6,#0076ff)_1]">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-0.4 sm:px-7 lg:px-2">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            <img src="/logotipo.svg" alt="The V Society" className="h-25 md:h-25 object-contain" />
          </Link>
          ...
          <div className="hidden items-center gap-10 md:flex">
            {!isLandingPage && navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`group relative inline-flex items-center text-sm font-medium transition-colors ${isActive(item.href) ? 'text-[#0076ff]' : 'text-slate-200 hover:text-[#00F5FF]'
                  }`}
              >
                <span className="relative z-10">{item.label}</span>
                <span
                  className={`absolute left-0 bottom-[-2px] h-[2px] w-full origin-center scale-x-0 bg-[#0076ff] transition-transform duration-300 group-hover:scale-x-100 ${isActive(item.href) ? 'scale-x-100' : ''
                    }`}
                />
              </Link>
            ))}
          ...
          <div className="flex flex-col gap-3 rounded-2xl border border-slate-800/70 bg-[#030712]/95 p-4 shadow-[0_0_18px_rgba(0,245,255,0.08)]">
            {!isLandingPage && navItems.map((item) => (
              <Link
                ...
                className={`group inline-flex items-center justify-between rounded-xl px-3 py-3 text-sm font-medium transition-colors ${isActive(item.href) ? 'text-[#0076ff]' : 'text-slate-200 hover:text-[#00F5FF]'
                  }`}
              >
                <span>{item.label}</span>
                <span
                  className={`block h-[2px] w-full origin-center scale-x-0 bg-[#0076ff] transition-transform duration-300 group-hover:scale-x-100 ${isActive(item.href) ? 'scale-x-100' : ''
                    }`}
                />
              </Link>
            ))}
